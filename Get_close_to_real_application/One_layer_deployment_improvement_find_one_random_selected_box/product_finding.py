...
def random_walk(swarm,boxes):
    ...
    for i in range(boxes.num_boxes):
        for j in range(swarm.num_agents):
            if box_dist[i,j] > 30:
               F_box[i, 0, j] = 0
               F_box[i, 1, j] = 0
...
def avoidance(rob_c, map):  # input the agent positions array and the warehouse map
    ...
    for i in range(num_agents):
        if abs(difference_in_x[i,0]) > 30 and abs(difference_in_x[i,1]) > 30:
           Fy[i] = 0
        if abs(difference_in_y[i,2]) > 30 and abs(difference_in_y[i,3]) > 30:
           Fx[i] = 0
        ...
        if self.anim == False:
            while self.robots.counter < self.time:
                ...
                if list(set(self.robots.unioninform).intersection(set(self.items.random_selected_target))) == self.items.random_selected_target:
                    ...
                    print('number of target information in whole robot swarm is', self.repeat_information)
