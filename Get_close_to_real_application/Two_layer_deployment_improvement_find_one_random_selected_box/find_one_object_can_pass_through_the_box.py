...
R_wall = 35 # repulsion 'forces'/influence factors for robots-walls

obstacle_feel = 30 # the maximum distance between self and other things if robot can feel the force
marker_size = 300*0.5/20 #diameter
...
class boxes():
      def __init__(self, number_of_boxes,width_box):
          ...
          self.random_selected_target = np.random.randint(number_of_boxes,size=1)
class swarm():
      def __init__(self,num_agents,scan_scope,width_swarm):
...
def random_walk(swarm,boxes):
    ...
    for i in range(swarm.num_agents):
        for j in range(swarm.num_agents):
            if agent_distance[i,j] > obstacle_feel:
               F_agent[i, 0, j] = 0
               F_agent[i, 1, j] = 0
...
def avoidance(rob_c, map):  # input the agent positions array and the warehouse map
    ...
    for i in range(num_agents):
        if abs(difference_in_x[i,0]) > obstacle_feel and abs(difference_in_x[i,1]) > obstacle_feel:
           Fy[i] = 0
        if abs(difference_in_y[i,2]) > obstacle_feel and abs(difference_in_y[i,3]) > obstacle_feel:
           Fx[i] = 0
...
class data:
    def __init__(self,num_agents,anim,limit,num_boxes,scan_scope,data_width,data_height):
        ...
        self.robots = swarm(self.num_agents,self.scan_scope,self.data_in_width)
        self.time = limit
        self.anim = anim
    ...
    def data_collect(self):
        ...
        if self.anim == False:
            while self.robots.counter <= self.time:
                self.robots.iterate(self.items)
                if list(set(self.robots.unioninform).intersection(set(self.items.random_selected_target))) == self.items.random_selected_target:
                    print('target customers want to get is', self.items.random_selected_target)
                    ...
                    #for i in range(len(self.robots.unioninform)):
                        #if self.robots.unioninform[i] == self.items.random_selected_target[0]:
                            #self.repeat_information = self.repeat_information + 1
                    self.repeat_information = len(np.argwhere(np.array(self.robots.unioninform) == self.items.random_selected_target))
                    self.repeat_information = self.repeat_information - 1
            ...
            print('product can not be found in limited time')
            return self.robots.counter,self.repeat_information
        if self.anim == True:
            self.ani()
    ...
    def ani(self):
        ...
        def animate(i):
            ...
            if list(set(self.robots.unioninform).intersection(set(self.items.random_selected_target))) == self.items.random_selected_target:
                ...
                for i in range(len(self.robots.unioninform)):
                    if self.robots.unioninform[i] == self.items.random_selected_target[0]:
                       repeat_information = repeat_information + 1
                print('number of target information in whole robot swarm is',repeat_information)
                print('union information is',self.robots.unioninform)
                exit()
